...
# Once a problem has been encountered, wait for a reply
def wait_for_reply(me, listening_socket, my_host, my_port):
    print(f"Elf {me} waiting for a reply")
    connection, _ = listening_socket.accept()
    msg = connection.recv(MAX_MSG_LEN).decode()

    if msg == "notify_santa":
        print(f"Elf {me} is in a group ready to see Santa and will notify Santa")
        notify_santa_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            notify_santa_socket.connect((DEFAULT_HOST, SANTA_PORT))
            notify_santa_socket.sendall(MSG_SORT_PROBLEM)
            notify_santa_socket.close()
        except Exception as e:
            print(f"Elf {me} failed to notify Santa: {e}")
